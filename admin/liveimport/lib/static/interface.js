...
function SelectorGadget() {
  ...
  this.border_padding = 2;
    // English ----------------------------
    this.lang_accept = 'Accept';
    this.lang_clear = 'Reset';
    this.lang_show = 'Show Hidden';
    this.lang_intro = 'Click on the page element to get';
    this.lang_get = 'Click to get this';
    this.lang_get2 = 'Click to get this';
    this.lang_fail = 'Impossible to find a selector for this element.';
    this.lang_skip = 'Click to skip this';
    this.lang_show_message = "All hidden elements on this page will be shown now. \n If some element closes the view for you, move mouse to it and press DEL.";
  this.b_top = null;
  this.b_left = null;
  this.b_right = null;
  this.b_bottom = null;
...
SelectorGadget.prototype.makeBorders = function(orig_elem, makeRed) {
  ...
  var height = elem.outerHeight()
    ...
    if (makeRed || elem.hasClass("sg_suggested") || elem.hasClass("sg_selected")) path_to_show = this.lang_skip;
    else
        if (this.path_output_field.value==this.lang_intro) path_to_show = this.lang_get2;
        else path_to_show = this.lang_get;
...
SelectorGadget.prototype.sgMouseover = function(e) {
  ...
  if (ctrl_pressed) return true;
  if (!document.body || !document.body) return false;
...
SelectorGadget.prototype.sgMousedown = function(e) {
    ...
    //console.log(e);
    if(!e.hasOwnProperty('originalEvent')) return true;
  var gadget = e.data.self;
  if (ctrl_pressed) return true;
  var elem = this;
  var w_elem = jQuery(elem);

  if (rsingle && !w_elem.hasClass('sg_border')) gadget.clearSelected();
  ...
  var prediction = gadget.prediction_helper.predictCss(gadget.selected, gadget.rejected.concat(gadget.restricted_elements));
    //MXS
    if (rsingle && !prediction) {
        alert(gadget.lang_fail);
        w_elem.removeClass('sg_selected');
        gadget.selected.splice(jQuery.inArray(elem, gadget.selected), 1);
    }
    //MXE
  ...
  //if (rsingle && prediction) SelectorGadgetSubmit(gadget);
...
SelectorGadget.prototype.setupEventHandlers = function() {
  ...
  jQuery("*:not(.sg_ignore)").bind("click.sg", { 'self': this }, this.sgMousedown);
...
SelectorGadget.prototype.suggestPredicted = function(prediction) {
  if (prediction && prediction != '') {
    ...
    if (this.clear_button) {
      if (count > 0) {
        this.clear_button.attr('value', this.lang_clear+' (' + count + ')');
      } else {
        this.clear_button.attr('value', this.lang_clear);
      }
    }
  }
};
...
SelectorGadget.prototype.setPath = function(prediction) {
  if (prediction && prediction.length > 0)
    ...
    this.path_output_field.value = this.lang_intro;
};
...
SelectorGadget.prototype.showEverything = function(e) {
    var self = (e && e.data && e.data.self) || this;
    parent.alert(self.lang_show_message);
    jQuery(':relative').show();
};
...
SelectorGadget.prototype.clearSuggested = function() {
  ...
  if (this.clear_button) this.clear_button.attr('value', this.lang_clear);
};
...
function SelectorGadgetSubmit(self) {
    var sel = self.path_output_field.value;
    if (sel==self.lang_intro) sel='';
    //if (sel.indexOf(',')+sel.indexOf(':')>0)
     //   sel = self.prediction_helper.cssToXPath(sel).replace(/"/g, "'");
    window.top.iSetSelector(sel);
}

SelectorGadget.prototype.makeInterface = function(hide) {



  this.sg_div = jQuery('<div>').attr('id', '_sg_div').addClass('sg_bottom').addClass('sg_ignore');
  if (hide) this.sg_div.css("display","none");
  var self = this;

    this.show_button = jQuery('<input class="button show" type="button" value="'+this.lang_show+'" />').bind("click", {'self': this}, this.showEverything).addClass('sg_ignore');
    this.sg_div.append(this.show_button);
  ...
  this.clear_button = jQuery('<input class="button clear" type="button" value="'+this.lang_clear+'" />').bind("click", {'self': this}, this.clearEverything).addClass('sg_ignore');
  ...
  this.sg_div.append(jQuery('<input class="button" type="button" value="'+this.lang_accept+'" />').bind("click", {'self': this},
      function () {
          SelectorGadgetSubmit(self);
      } ).addClass('sg_ignore'));
...
if (typeof(selector_gadget) == 'undefined' || selector_gadget == null) {
  (function() {
    ...
    selector_gadget.makeInterface(false);
    selector_gadget.clearEverything();
    selector_gadget.setMode('interactive');
    selector_gadget.analytics();
      //MX
      if (window.top.iSelector) {
          selector_gadget.path_output_field.value = window.top.iSelector;
          selector_gadget.refreshFromPath(selector_gadget.path_output_field);
      }
  })();
