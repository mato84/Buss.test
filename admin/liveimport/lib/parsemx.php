...
function curl_redir_exec($curl_handler, $file_name = false, $file_handler = false)
{
    ...
    if ($http_code >299 && $http_code < 400)
    {
        if ($file_name) {
            fclose($file_handler);
            unlink($file_name);
        ...
        if ($http_code != 307) {
            ...
            curl_setopt($curl_handler, CURLOPT_POSTFIELDS, false);
        }
...
function tags_image($selector, $source = false) {
    ...
    if (!$images) { // No regular image-looking links
        foreach ($hts as $ht) {
                $img = tags_attr('img','src',$ht);
                foreach ($img as $im) {
                    $im = url($im);
...
function page_var($var_name, $source = false, $single = true) {

    if ($source === false) { global $http_html; $source = $http_html; } // Use set_source() default
    $start = 0;
    $re = array();
    while ($p = strpos($source, $var_name, $start)) {
        $start = $p + strlen($var_name);
        if (ctype_alnum($source[$p-1])) continue;
        if ($source[$p-1] == '"' || $source[$p-1] == "'") $start++;
        $p = $start;
        while (ctype_space($source[$p])) $p++;
        if ($source[$p] != ':' && $source[$p] != '=') continue;
        $p++;
        while (ctype_space($source[$p])) $p++;
        $closer = ';';
        ...
        if (!$p2) continue;
        $var = substr($source, $p, $p2-$p);
        if ($closer == '"' || $closer == "'") $var = stripslashes($var);
        if ($single) return re($var);
        $re[] = $var;
    }
    return re($re);
}

function page_vars($var_name, $source = false) {
    return re(page_var($var_name, $source, false));
}
...
class nokogiri2 implements IteratorAggregate{
    ...
    public function getXpathSubquery($expression, $rel = false, $compile = true){
        ...
        static $nesting, $original_expression;
        $root = false;
        if (!$original_expression) {
            ...
            if ($nesting++ > 30) {
                xnotice("Failed to build query for \"<strong>$expression</strong>\"");
                return false;
            }
        }
        ...
        if ($root) $original_expression = false;
        return $query;
    }
