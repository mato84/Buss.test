...
class ModelCatalogWaypoint extends Model {
  ...
  public function addWaypoint($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "way_point SET name = '" . $this->db->escape($data['name']) . "', sort_order = '" . (int)$data['sort_order'] . "',city_id = '" . (int)$data['city_id'] . "',time = '" . $data['time'] . "',place = '" . $this->db->escape($data['place']) ."',manufacturer_id = '" . $data['manufacturer_id'] . "'");
  ...
  public function editWaypoint($waypoint_id, $data) {
    ...
    $this->db->query("UPDATE " . DB_PREFIX . "way_point SET name = '" . $this->db->escape($data['name']) . "', sort_order = '" . (int)$data['sort_order'] ."',city_id = '" . (int)$data['city_id'] . "',time = '" . $data['time'] ."',place = '" . $this->db->escape($data['place']) . "' WHERE waypoint_id = '" . (int)$waypoint_id . "'");
